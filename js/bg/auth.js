...
/* export default*/ class Auth {
    constructor(utils, csrfToken = null, authorization = null, since = null) {
      this.utils = utils
    ...
    async testAuth(){
      ...
      if (this.isAuth()){
        try
        {
          ...
          {
            this.utils.setData({user_info: res})
            this.user_info = res
            _user_info = res
    ...
    async getAuth() {
      ...
      // otherwise try to load from storage and test if it's good
        ...
        let data = await this.utils.getData("auth");
    ...
    async getNew(){
      ...
      if (reload_ok){
        chrome.tabs.reload(this.utils.tabId);
      }
    }
    ...
    async updateAuth(csrfToken, authorization) {
      ...
      this.utils.setData({ auth: auth_params})
    }
