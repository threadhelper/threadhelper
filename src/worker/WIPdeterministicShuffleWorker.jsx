...
const searchIndex$ = _searchIndex$.bufferWhileBy(isMidSearch$).map(last) // searchIndex$ :: msg
const searchResultsIds$ = searchIndex$.map(pipe(tap(_=>emitMidSearch(true)), msg2SearchArgs(getIndex), args=>search(...args))).toProperty() // searchResults$ :: id
const searchResults$ = promiseStream(searchResultsIds$, getTweetsFromDbById)


const getDefaultTweets$ = makeMsgStream('getDefaultTweets') // msg
getDefaultTweets$.log('getDefaultTweets$')
const filterChange$ = makeMsgStream('filterChange') // msg
filterChange$.log('filterChange$')
const defaultChangeMsg$ = Kefir.merge([getDefaultTweets$, filterChange$]) // msg
defaultChangeMsg$.log('defaultChangeMsg$')
const defaultTweetsSample$ = promiseStream(getDefaultTweets$, m=>getRandomSample(getKeys, m.n_tweets)) // [id]
defaultTweetsSample$.log('defaultTweetsSample$')
const reqDefaultChange$ = Kefir.combine([defaultChangeMsg$, defaultTweetsSample$],(a,b)=>{return {...a, ids: b}}) // msg w/ [id]
reqDefaultChange$.log('reqDefaultChange$')


const defaultTweets$ = promiseStream(reqDefaultChange$, m=>filterSample(db.get(getDb()), m.filters, m.screen_name, m.ids)) // [tweet]
defaultTweets$.log('defaultTweets$')
...
const getKeys = ()=>getDb().getAllKeys('tweets')
...
const msg2SearchArgs = curry((_getIndex, m) => {const index = _getIndex(); return [m.filters, m.username, m.n_results, _getIndex(), m.query]})
// const searchIndex = pipe( // async
//   // inspect('search'),
//   tap(_=>emitMidSearch(true)),
//   msg2SearchArgs(getIndex),
//   args => search(...args),
//   andThen(pipe(
//     getTweetsFromDbById,
//     andThen(pipe(
...
const msg2SampleArgs = m=>[m.n_tweets, m.filters, db.get(getDb()), m.screen_name, getKeys]
...
const makeSearchResponse = pipe(assoc('res', __ , {type:'searchIndex',}), tap(_=>emitMidSearch(false)))
subReq(searchResults$, makeSearchResponse)
subReq(defaultTweets$, assoc('res', __ , {type:'getDefaultTweets',}))
