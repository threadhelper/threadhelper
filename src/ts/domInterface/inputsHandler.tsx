...
import { getMode, getTweetId, isFocused, elIntersect, elContained } from '../utils/wutils';
import { msgBG } from '../utils/dutils';
import Kefir, { Observable, Property, sequentially } from 'kefir';
import { flattenModule, inspect, currentValue } from '../utils/putils';
...
export function buttonClicked(target: EventTarget | Element, selector: string, parent: ElParent = null): boolean {
    parent = parent == null ? document : parent;
    // return [...parent.querySelectorAll(selector)].some((el)=> containsOrContained(target, el))
    return Array.from(parent.querySelectorAll(selector)).some((el: {contains: (arg0: any) => unknown;}) => el.contains(target));
}
const clickCondStream = curry((parent: ElParent, selector: string, e: Event): boolean => buttonClicked(e.target, selector, parent));
const docClickCondStream = clickCondStream(document);
// Cond fromClick :: Cond -> Stream (Event)
const fromClick = (cond: InputCond) => Kefir.fromEvents<MouseEvent | KeyboardEvent,Error>(document.body, 'click').filter(cond);
// Cond fromShortcut :: Cond -> Stream (Event)
const fromShortcut = (cond: InputCond) => Kefir.fromEvents<MouseEvent | KeyboardEvent,Error>(document.body, 'keydown').filter(cond);
// Stream streamInput :: (Cond -> Cond) -> Stream (Event)
const inputStream = curry((shortCond: {
    (e: any): any;
...
export function makePostStream() {
    // Cond :: Event -> Bool
    const tweetShortCond = (e: KeyboardEvent): boolean => (e.ctrlKey && e.key === 'Enter' && isComposeFocused());
...
export function makeComposeFocusObs() : Observable<FocusEvent, any>[]{
    ...
    const focusIn = Kefir.fromEvents<FocusEvent, any>(document.body, 'focusin').filter(_ => getMode() != UrlModes.other).filter((e:FocusEvent) =>{
        console.log('[DEBUG] makeComposeFocusObs', {activeElement:document.activeElement, event: e.target} ); 
        return elContained(editorSelector, e.target as Element)}); //.map(_=>'focused')
    const focusOut = Kefir.fromEvents<FocusEvent, any>(document.body, 'focusout').filter(_ => getMode() != UrlModes.other).filter((e:FocusEvent) => elIntersect(editorSelector, e.target as Element));//.map(_ => 'unfocused') ;
...
export function makeReplyObs(mode$: Observable<UrlModes, Error>)  {
    ...
    const replyShortCond = (e: {key: string;}) => (e.key === 'r' && isTweetCardFocused());
    // @ts-expect-error ts-migrate(2345) FIXME: Argument of type '(e: {    key: string;}) => any' ... Remove this comment to see the full error message
...
export const makeLastClickedObs = () => fromClick(docClickCondStream('article *'));
// getHostTweetId :: lastStatus$ -> event -> id
// Gets the id of the tweet on which an action  (the event) is being performed
export const getHostTweetId = curry((lastStatus$: curProp<UrlModes>, e:Event): string => {
    // if(!(e != null)) return null //if null, we're not replying to anything
    return pipe(prop('target'), actionEl2TweetEl, ifElse(isNil, _ => null, ifElse(hasDate, getTweetId, _ => lastStatus$.currentValue())))(e);
});
