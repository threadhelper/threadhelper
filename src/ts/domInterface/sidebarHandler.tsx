...
import ThreadHelper from '../components/ThreadHelper.tsx';
// @ts-expect-error ts-migrate(2691) FIXME: An import path cannot end with a '.tsx' extension.... Remove this comment to see the full error message
import { getMode, isSidebar } from '../utils/wutils.tsx';
// @ts-expect-error ts-migrate(2691) FIXME: An import path cannot end with a '.tsx' extension.... Remove this comment to see the full error message
import { getData, setData } from '../utils/dutils.tsx';
// @ts-expect-error ts-migrate(2691) FIXME: An import path cannot end with a '.tsx' extension.... Remove this comment to see the full error message
import { flattenModule } from '../utils/putils.tsx';
...
function makeNewDummy(){
    const dummyUI : Element = document.createElement('div'); dummyUI.className = "dummyContainer";
    const dummyLeft : Element = document.createElement('div'); dummyLeft.className = "dummyLeft";
    const dummyRight : Element = document.createElement('div'); dummyRight.className = "dummyRight"; dummyRight.id = "suggestionContainer";
...
function makeDummyCompose(thBar: Element) : Element | null {
    let dummies : Element[] = Array.from(document.getElementsByClassName('dummyContainer'));
...
// @ts-expect-error ts-migrate(2691) FIXME: An import path cannot end with a '.tsx' extension.... Remove this comment to see the full error message
import { obsAdded, obsRemoved } from '../utils/kefirMutationObs.tsx';
// @ts-expect-error ts-migrate(2614) FIXME: Module '"../../node_modules/@types/kefir"' has no ... Remove this comment to see the full error message
...
export function makeHomeSidebarObserver(thBar: EventTarget | NodeJS.EventEmitter | {on: Function; off: Function;}) {
    ...
    console.log('[DEBUG] makeHomeSidebarObserver 1', {thBar, trendAdd$, sidebarOutDoc$, render$, unrender$, homeSidebarObserver$} )
...
export function makeFloatSidebarObserver(thBar: EventTarget | NodeJS.EventEmitter | {on: Function; off: Function;}) {
