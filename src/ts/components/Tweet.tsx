...
export function Tweet(props: { tweet: thTweet; score?: number}){
...
let onClickTweet = function(tweet: { id: any; }, getActiveComposer: () => any, e: { target: any; }){
...
function getTimeDiff(time: string | number | Date) {
  let now = new Date()
  let timeDate = new Date(time)
  // FIXME: The left-hand side of an arithmetic operation must... Remove this comment to see the full error message
...
function replaceBetween(originalString: string, start: number, end: number, replacement: string){
    return originalString.substr(0,start)+replacement+originalString.substr(end);
}

function reformatText(text, reply_to=null, _mentions=null, urls=null, media=null) {
