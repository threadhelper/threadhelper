...
const usePageN = (n0: number, nMax: number) => {
  ...
  const decPage = () => {
    page <= n0 ? null : setPage(page - 1);
  };
  const incPage = () => {
    page >= nMax ? null : setPage(page + 1);
  };
...
const VideoTutorial = () => {
  ...
  return (
    <iframe
      width="100%"
      src="https://www.youtube-nocookie.com/embed/voCpNvldCjk"
      frameBorder="0"
...
const Tutorial = (props) => {
    ...
    [
      {
        text: (
          <span>
            Welcome to Thread Helper! Just open Twitter and you should see our
            sidebar. Have feedback?{' '}
            <a href="#" onClick={(_) => setPage(pages.length)}>
              Click here and write it down.
            </a>
          </span>
        ),
        img: VideoTutorial(),
      },
      {
        text: (
          <span>
            {
              'ThreadHelper is a new app that finds you the tweets you need as you type.'
            }
          </span>
        ),
        img: getImage(img1),
      },
      {
        text: (
          <span>
            {
              "As you type, the results on the sidebar change. You can click them to copy their link and easily paste them into the tweet you're writing to generate a quote tweet."
            }
          </span>
        ),
        img: getImage(img2),
      },
      {
        text: (
          <span>
            {'ThreadHelper has 3 filters: Retweets, Bookmarks, and Replies.'}
          </span>
        ),
        img: getImage(img3),
      },
      ...
      //  img:getImage(img6)},
      {
        text: (
          <span>
            {
              'You can upload your archive by clicking "(load archive)". It\'s stored on your machine, so we never see it ourselves.'
            }
          </span>
        ),
        img: getImage(img7),
      ...
      //  {text:<span>Thread Helper! 🧵 <a href="#" onClick={_=>setPage(1)}>Click here to follow the tutorial again.</a></span>,
      {
        text: (
          <span>
            Thread Helper!{' '}
            <a href="#" onClick={(_) => setPage(1)}>
              Click here to follow the tutorial again.
            </a>
          </span>
        ),
        img: null,
  ...
  return (
    <div id="tutorial-window">
      <div id="tutorial-nav">
        <div id="tutorial-prev" className="nav-arrow float-right">
          <a href="#" onClick={decPage}>
            {'< '}
          </a>
        </div>
        <div id="tutorial-nav-next" className="nav-arrow float-right">
          <a href="#" onClick={incPage}>
            {' >'}
          </a>
        </div>
        <div id="tutorial-nav-index" className="float-right">
          {page}/{pages.length}
        </div>
      </div>
      <div id="block1">
        ...
        <TutorialPage
          ...
          img={pages[page - 1].img}
        ></TutorialPage>
      </div>
      <div id="block2">{page >= pages.length ? <FeedbackForm /> : null}</div>
    </div>
  );
};
