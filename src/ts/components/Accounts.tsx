...
const makeAccItems = pipe(values, map((acc: FullUser )=> { return { id: acc.id_str, screen_name: acc.screen_name, leftIcon: accountFilterAvi(acc.profile_image_url_https), effect: () => { } }; }));
...
//   
    ...
    return (
        ...
        <div class={`options icon-button`}> 
            <AccountIcon class={`account-icon hoverHighlight  ${sync ? 'synced' : 'unsynced'}`} onClick={onClickButton} onBlur={closeMenu}></AccountIcon> 
        </div>
        <span class="tooltiptext"> {syncDisplay} </span> 
        {open && <DropdownMenu name={'Accounts'} componentItems={[]} filterItems={makeAccItems(activeAccounts)} items={[]} debugItems={[]} closeMenu={() => setOpen(false)} itemClickClose={false}/>}
    </div>);
}
