...
const domReady = constant(document.readyState)
  ...
  .merge(
    fromEvents(document, 'DOMContentLoaded').map((_) => document.readyState)
  )
  .take(1)
  .toProperty();

export const mutationRecorder = (root: Node, options: MutationObserverInit) => {
  return domReady.flatMap((_) =>
    stream((emitter) => {
...
const matches = (selector) => (el: { matches: (arg0: any) => any }) =>
  el.matches && el.matches(selector);
const pluck = (key: string) => (arr: any[]) =>
  arr.map((el: { [x: string]: any }) => el[key]);
const noop = (x) => x;
const concat = (acc: string | any[], n) => acc.concat(n);
const nodesMatching = (selector) => (nodes: any[]) =>
  nodes.filter(matches(selector));
const toArray = (collection) => Array.prototype.concat.apply([], collection);
const flattenArrays = (nodeArrs: any[]) =>
  nodeArrs.map(toArray).reduce(concat, []);
...
const includeNested = (selector) => (nodes: any[]) => {
  const m = nodes
    ...
    .map((node: { querySelectorAll: (arg0: any) => any }) =>
      toArray(node.querySelectorAll(selector))
    )
    .reduce(concat, []);
...
export const obsAdded = (root: Node, selector, subtree) => {
  return mutationRecorder(root, {
    ...
    .merge(
      constant(
        Array.prototype.concat.apply([], document.querySelectorAll(selector))
      )
    )
    .filter(populated);
};
