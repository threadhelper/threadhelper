import { __, curry, pipe, andThen, map, filter, reduce, tap, apply, tryCatch, T} from 'ramda' // Function
...
//project utilities
export const flattenModule = (window: { [x: string]: unknown; },R: ArrayLike<unknown> | { [s: string]: unknown; })=>Object.entries(R).forEach(([name, exported]) => window[name] = exported);
export const delay = (ms: number) => new Promise(res => setTimeout(res, ms));
export const inspect = curry ((prepend, data)=>{console.log(prepend, data); return data;})
export const wInspect = curry ((log, prepend, data)=>{log(prepend, data); return data;})
...
export const timeFn = curry((text, fn: (...arg0: any) => any)=>{return (...args)=>{console.time(`[TIME] ${text}`); const res = fn(...args); console.timeEnd(`[TIME] ${text}`); return res}})
export const wTimeFn = curry((log, text, fn: (...args: any) => any) => {return (...args) => {
...
// stream utilities
export const currentValue = function(this:Observable<any,Error>) { //doesn't really belong in putils, should be a dedicated filefor extending kefi
  var result;
  var save = function(x) {
    result = x;
  };
...
export const streamAnd = (streams:Observable<any,any>[]) => Kefir.combine(streams, (...args)=>reduce(and, true, args))
export const toVal = (val,stream: Observable<any,any>) => stream.map(()=>val).toProperty(()=>val)
...
export const promiseStream = curry(<T,S>(promise_fn:(arg0:T) => Promise<S>, stream:Observable<T,any>): Observable<S,any> => {return stream.flatMapLatest(x=>Kefir.fromPromise(promise_fn(x)))})
export const waitFor = curry(<T,>(waited: Observable<any,any>, waiter: Observable<T,any>): Observable<T,any> => waiter.bufferWhileBy(waited).flatten()) // makeSafe :: Stream -> Stream
export const makeMsgStream = curry((msg$: Observable<Msg,any>, name: string): Observable<Msg,any> => msg$.filter(propEq('type', name)))
// export const makeMsgStreamSafe = (msg$, name) => makeSafe(makeMsgStream(msg$, name)) // makeMsgStreamSafe :: String -> Stream msg
export const errorFilter = curry(<T, >(name:string, stream: Observable<T, any>): Observable<T, any> => stream.filterErrors(e=>{console.trace(`[ERROR] from ${name}`, {stream, e});  return false}));
...
export const nullFn = ()=>{}
export const renameKeys = (keysMap: { [x: string]: any; }) => (obj: ArrayLike<unknown> | { [s: string]: unknown; }) => Object.entries(obj).reduce(
