...
import { queue_load } from '../utils/params';
...
export const compareAuths = (a: Credentials, b: Credentials) => {
  return (
    a.authorization == b.authorization && a['x-csrf-token'] == b['x-csrf-token']
  );
};
export const validateAuthFormat = (x: Credentials) =>
  prop('authorization', x) != null && prop('x-csrf-token', x) != null;

//chrome storage
export const isOptionSame = curry(
  (name: string | number, x: { oldVal: any; newVal: any }) =>
    (isNil(x.oldVal) && isNil(x.newVal)) ||
    (!isNil(x.oldVal) &&
      !isNil(x.newVal) &&
      path(['oldVal', name, 'value'], x) === path(['newVal', name, 'value'], x))
);
// makeOptionsObs :: String -> a
export const _makeOptionObs = curry(
  async (
    optionsChange$: Observable<StorageChange, any>,
    itemName: string
  ): Promise<Observable<Option, any>> => {
...
export const _subWorkQueueStg = curry(
  (subscriptions, storageChange$, name, workFn) => {
    // Waiting queue
    ...
    const queue$ = makeInitStgObs(storageChange$, name).filter(
      pipe(isNil, not)
    );
    queue$.log(name + '$');
