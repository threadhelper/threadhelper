...
import { getData, setData, makeOnStorageChanged } from '../utils/dutils.tsx';
// @ts-expect-error ts-migrate(2691) FIXME: An import path cannot end with a '.tsx' extension.... Remove this comment to see the full error message
import { flattenModule, inspect, renameKeys } from '../utils/putils.tsx';
...
export const apiToTweet = (entry) => {
    let tweet = {};
    (tweet as any).retweeted = isNil(prop('retweeted', entry)) ? false : prop('retweeted', entry);
    let rt_entry = entry;
    ...
    (tweet as any).text = unescape(prop('full_text', rt_entry) || prop('text', rt_entry));
    (tweet as any).profile_image = path(['user', 'profile_image_url_https'], rt_entry);
    // Add full quote info.
    if ((tweet as any).has_quote && (tweet as any).is_quote_up && prop('quoted_status', entry)) {
        ...
        (tweet as any).quote = {
            ...
            mentions: defaultTo([], path(['entities', 'user_mentions'], quoted_status).map((x: {
                screen_name: any;
                indices: any;
            }) => ({ username: x.screen_name, indices: x.indices }))),
            // URLs.
            // @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
            urls: path(['entities', 'urls'], quoted_status).map((x: {
                url: any;
                display_url: any;
                expanded_url: any;
            }) => ({ current_text: x.url, display: x.display_url, expanded: x.expanded_url })),
            has_media: typeof path(['entities', 'media'], quoted_status) !== "undefined",
            media: null,
        };
        if ((tweet as any).quote.has_media) {
            // @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
            (tweet as any).quote.media = path(['entities', 'media'], quoted_status).map((x: {
                url: any;
                media_url_https: any;
            }) => ({ current_text: x.url, url: x.media_url_https }));
        }
    }
    return tweet;
};
// @ts-expect-error ts-migrate(2571) FIXME: Object is of type 'unknown'.
const findAuthor = (rt: string[], t) => path(['entities', 'user_mentions'], t).find(t => { return prop('screen_name', t).toLowerCase() === rt[1].toLowerCase(); });
...
const makeQuote = quoted_status => {
    return {
        ...
        media: defaultTo([], path(['entities', 'media'], quoted_status)).map((x: {
            url: any;
            media_url_https: any;
        }) => ({ current_text: x.url, url: x.media_url_https })),
    };
};
const _isOwnTweet = (rt: any[], user_info) => (isNil(rt) || rt[1] === prop('screen_name', user_info));
...
// @ts-expect-error ts-migrate(2769) FIXME: No overload matches this call.
path(['entities', 'user_mentions']), pipe(path(['entities', 'user_mentions']), map(x => ({ username: prop('screen_name', x), indices: prop('indices', x) }))), _ => []);
...
// @ts-expect-error ts-migrate(2769) FIXME: No overload matches this call.
path(['entities', 'urls']), pipe(path(['entities', 'urls']), map(x => ({ current_text: prop('url', x), display: prop('display_url', x), expanded: prop('expanded_url', x) }))), _ => []);
...
export const validateTweet = (t:object): boolean => {
    ...
    if (!valid)
        console.log('ERROR invalid tweet', valid);
    return !isNil(valid);
};
...
const overlap = (minNew, maxNew, currentIds: readonly unknown[]) => pipe(sortKeys, dropLastWhile(gtId(__, maxNew)), dropWhile(ltId(__, minNew)))(currentIds);
// findInnerDiff :: [id] -> [id] -> [id] // finds currentIds ids in the range [min, max](incoming) which are missing in incomingIds
export const findInnerDiff = curry((currentIds: readonly unknown[], incomingIds: readonly unknown[]) => {
