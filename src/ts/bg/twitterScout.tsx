...
export const thFetch = async (url: string, auth: RequestInit): Promise<any> => fetch(url, auth)
    ...
    .catch(error => {
        switch(error.status){
            case 88: console.error(`[ERROR] thFetch ${error.status}, ${error.statusText}`, {error, auth, url})
                break;
            case 401: console.error(`[ERROR] thFetch ${error.status}, ${error.statusText}`, {error, auth, url})
                break; // Unauthorized client, lacks valid auth for target resource. Re-auth might make a difference
            case 403: console.error(`[ERROR] thFetch ${error.status}, ${error.statusText}`, {error, auth, url})
                break; // Forbidden, server understood but refuses to authorize. re-auth will make no difference.
            case 429: console.error(`[ERROR] thFetch ${error.status}, Too Many Requests`, {error, auth, url})
                throw error // Too many requests.
            default: 
                console.error(`[ERROR] thFetch ${error.status}, ${error.statusText}`, {error, auth, url})
                throw error
        }
    });
...
const makeApiSearchUrl = q => `https://twitter.com/i/api/2/search/adaptive.json?q=${encodeURIComponent(q)}`
...
const getApiTweets = pipe(path(['globalObjects', 'tweets']), x => Object.values(x));
...
const assocQTs = curry((qts: {[x: string]: Tweet;}, tweet) => when(
    pipe(prop('quoted_status_id_str'), isNil, not), 
    pipe(assoc('quoted_status', qts[tweet.quoted_status_id_str]), assoc('is_quote_status', true))
    )(tweet));
...
const getQTs = curry((getAuthInit: () => Object, tweets: Tweet[]) => pipe(
    ()=>tweets,
    getApiTweets, 
    map(getQtId), 
...
export async function getBookmarks(getAuthInit: () => RequestInit):Promise<Tweet[]> {
    ...
    const makeRes = (bookmarks:any[]):{} => pipe(getApiTweets, map(assocUser(users)), map(assocQTs(qts)), inspect('bookmarks with qts'))(bookmarks);
    const res = await makeRes(bookmarks);
    return values(res);
}

export const searchAPI = curry(async (getAuthInit: () => RequestInit, query:string):Promise<Tweet[]> =>{
    ...
    const qts = await loopRetry(authGetQTs);
    const makeRes = (_res:any[]):{} => pipe(getApiTweets, map(assocUser(users)), map(assocQTs(qts)), inspect('_res with qts'))(_res);
