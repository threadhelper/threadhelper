...
export const loadIndex = (loadedIndexJson: elasticlunr.SerialisedIndexData<IndexTweet>) => isNil(loadedIndexJson) ?  makeIndex() : elasticlunr.Index.load(loadedIndexJson)

  

// IMPURE
export const addToIndex = R.curry( async (index: elasticlunr.Index<IndexTweet>, tweets: thTweet[]): Promise<elasticlunr.Index<IndexTweet>> => {
  const toDoc = R.pipe<thTweet[],any,IndexTweet[]>(values, project(tweet_fields));
  toDoc(tweets).forEach((x: IndexTweet)=>index.addDoc(x))
  return index
})

export const removeFromIndex = R.curry (async (index: elasticlunr.Index<IndexTweet>, tweet_ids: string[]) => {
  tweet_ids.forEach(x=>index.removeDocByRef(x)); 
  return index;
})

export const updateIndex = R.curry (async (index, tweets_to_add, ids_to_remove) => {
...
// 
//** Find related tweets */
export const getRelated = R.curry( async (index: elasticlunr.Index<IndexTweet>, query: string): Promise<elasticlunr.SearchResults[]> => {
...
const getDoc = R.curry((index: elasticlunr.Index<IndexTweet>, ref:elasticlunr.DocumentReference): IndexTweet => index.documentStore.getDoc(ref))
const isFull = R.curry((n_tweets: number, latest: string | any[]) => latest.length >= n_tweets) 
...
export const getTopNResults = R.curry(async (n_tweets: number, index: elasticlunr.Index<IndexTweet>, filters: SearchFilters, accsShown: User[], results: Promise<elasticlunr.SearchResults[]>): Promise<IndexSearchResult[]> => {
  const isValidTweet = makeValidateTweet(filters, accsShown)
...
export const search = R.curry (async (filters, accsShown, n_tweets, index, query): Promise<IndexSearchResult[]> => {
