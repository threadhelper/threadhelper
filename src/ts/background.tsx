...
export async function main() {
  ...
  //      Analytics
  const csGaEvent$ = msgStream('gaEvent').map(prop('event'));
  const csGaException$ = msgStream('gaException').map(prop('exception'));
  //      Storage
  ...
  const pWorkerMsg$ = wMsgEvent$.filter(
    propSatisfies((x) => R.type(x) == 'Object', 'data')
  ); // msgs from Promise Worker
  ...
  const reqSemanticSearch$ = wordSearchQuery$
    ...
    .map(last);
  const reqSearch$ = Kefir.merge([reqFullTextSearch$, reqSemanticSearch$]).thru(
    waitFor(notReady$)
  );
  ...
  const defaultsWorkerMsg$ = Kefir.combine(
  ...
  const filteredDefaultTweets$ = Kefir.combine(
  ...
  const searchWorkerMsg$ = Kefir.combine(
    [searchMode$, searchFilters$, accsShown$, reqSearch$],
    makeReqSearchMsg
  );
