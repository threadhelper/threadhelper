...
const activateSidebar = curry((inject: (arg0: Element) => any, bar: Element) => {
...
const deactivateSidebar = (bar: Element) => { render(null, bar); };
// Webpage events
const makeIdObsMsg = curry((lastClickedId$:curProp<any>, type) => { return { type: type, id: lastClickedId$.currentValue() }; });
...
const subObs = <T,>(obs:Observable<T,Error> | Property<T,Error|any>, effect:(val:T)=>void): Subscription => rememberSub(obs.observe({ value: effect }));
...
async function onLoad(thBarHome: Element, thBarComp: Element) {
    ...
    const urlChange$ = (gotMsg$.filter(propEq('type',"tab-change-url")) as unknown as Observable<UrlMsg,Error>).map(prop('url'));
    const mode$ = urlChange$.map(getMode);
    //      storage
    const storageChange$ = makeStorageChangeObs();
    const latest$ = storageChange$.filter((x: { itemName: string; }) => x.itemName == 'latest_tweets').map(prop('newVal'));
    const sync$ = storageChange$.filter((x: { itemName: string;} ) => x.itemName == 'sync').map(prop('newVal'));
    const syncDisplay$ = storageChange$.filter((x: { itemName: string; }) => x.itemName == 'syncDisplay').map(prop('newVal')).toProperty(() => '');
    //      webpage events
    //          theme
    ...
    const lastClickedId$ = makeLastClickedObs().map(getTargetId).filter(x => !isNil(x)).toProperty() as unknown as curProp<string>;
    const makeIdMsg = makeIdObsMsg(lastClickedId$); // function
    //          actions
    const actions$ = makeActionStream(); // post, rt, unrt
    const replyTo$ = makeReplyObs(mode$).map(getTargetId).toProperty(() => null) as curProp<string>;
    ...
    const composeUnfocused$ = Kefir.merge([composeFocusOut$.map(_ => ''), urlChange$.map(_ => '')]);
    const composeContent$ = composeFocus$.flatMapLatest((e: Event) => makeComposeObs(e.target as HTMLElement));
    composeContent$.log('composeContent$')
    const composeQuery$ = Kefir.merge([urlChange$.map(_ => ''), composeContent$]).toProperty(() => '');
    ...
    // };
    // Sidebar control
    const updateFloat = (value:any) => value ? activateFloatSidebar() : deactivateSidebar(thBarComp); //function
    const updateHome = (value:any) => value ? activateHomeSidebar() : deactivateSidebar(thBarHome); //function
