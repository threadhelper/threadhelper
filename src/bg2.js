...
class TweetWiz{
    ...
    async init(){
      ...
      getData("user_info").then((info)=>{this.user_info = info != null ? info : {}}),
      getData("has_archive").then((info)=>{this.has_archive = info != null ? info : false}),
      getData("has_timeline").then((info)=>{this.has_timeline = info != null ? info : false}),
      getData("tweets_meta").then((meta)=>{this.tweets_meta = meta != null ? meta : this.makeTweetsMeta(null)}),
      getData("latest_tweets").then((latest_tweets)=>{this._latest_tweets = latest_tweets != null ? latest_tweets : [];})
      // getData("user_queue").then((user_queue)=>{this._user_queue = user_queue != null ? user_queue : [];})
      // getData("pic_tweet_queue").then((pic_tweet_queue)=>{this._pic_tweet_queue = pic_tweet_queue != null ? pic_tweet_queue : {}})
      // getData("profile_pics").then((profile_pics)=>{this._profile_pics = profile_pics != null ? profile_pics : {};})
      ]).then(
        ()=>{
    ...
    async loadOptions(){
      getData("options").then(opts=>{
        this.options = opts != null ? opts : this.options
      })
    }

    get query_ready(){
      console.log(`query ready? ${wiz.user_info.screen_name} - ${auth.csrfToken}`)
    ...
    set user_info(user_info){
      if (user_info != this._user_info)
        ...
        console.trace("changing user info")
        setData({user_info:this._user_info})
		}
    ...
    set has_archive(has_archive){
      if (has_archive != this._has_archive)
        ...
        setData({has_archive:this._has_archive})
		}
    ...
    set has_timeline(has_timeline){
      if (has_timeline != this._has_timeline)
        ...
        this.updateSync()
        setData({has_timeline:this._has_timeline})
    }
    ...
    set tweets_meta(tweets_meta){
      console.log(`setting tweets meta ${JSON.stringify(tweets_meta,null,2)}`)
      this._tweets_meta = tweets_meta
      setData({tweets_meta:tweets_meta}).then(()=>{
      })
    }
    ...
    set latest_tweets(latest_tweets){
      ...
      setData({latest_tweets:this.latest_tweets}).then(()=>{
    ...
    async updateTweets(tweets_to_add, ids_to_remove){
      ...
      console.log('updateTweets', {tweets_to_add, ids_to_remove, n_new_tweets})
      ...
      if(n_new_tweets > 0 || n_deleted_tweets > 0){
        ...
        if(n_new_tweets > 0) { 
          this.tweets_meta = this.updateMeta(tweets_to_add, this.tweets_meta.count + n_new_tweets)
        }
        // // console.time("getLatest")
        if(n_deleted_tweets > 0 && n_new_tweets <= 0){
          let keys = await utils.db.getAllKeys('tweets')
          // console.time("sortKeys")
          this.tweet_ids = this.sortKeys(keys)
    ...
    // async loadUserInfo(){
    //   getData("user_info").then((info)=>{this.user_info = info})
...
class NLP{
  ...
  async search(_query, n_tweets = 20){
    ...
    if(wiz.tweet_ids.length <= 0 || !(this.index != null)){
       console.log("error: searching before tweets loaded"); 
       return []
    }
